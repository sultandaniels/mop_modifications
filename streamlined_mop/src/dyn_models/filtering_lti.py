...
def gen_A(elow, ehigh,
          n):  # generates a 2d A matrix with evalue magnitudes in between elow and ehigh. For matrices larger than 2d, when there are complex evalues it just ensures that all evalues are below ehigh and at least one evalue is above elow.
    if elow > ehigh:
    ...
    if np.iscomplex(eigs).any():  # if there are complex evalues
        ...
        if (np.sum(np.abs(lin.eigvals(out)) > elow - eps) < 1) or np.any(np.abs(lin.eigvals(
                out)) > ehigh + eps):  # if there are no evalues above elow or there are evalues above ehigh
        ...
        A = ((b - a) / (eigs[0] - eigs[-1])) * (mat - eigs[-1] * np.eye(n)) + a * np.eye(
            n)  # subtract the smallest evalue from the matrix
        ...
        if (np.any(np.abs(lin.eigvals(out)) < elow - eps) or np.any(
                np.abs(lin.eigvals(out)) > ehigh + eps)):  # if there are evalues below elow or above ehigh
            print("np.abs(lin.eigvals(out)):", np.abs(lin.eigvals(out)))
...
class FilterSim:
    ...
    def __init__(self, nx, ny, sigma_w, sigma_v, tri, C_dist, n_noise, new_eig):
        ...
        if tri == "upperTriA":
            ...
            A = np.diag(
                [0.99, 0.98, 0.97, 0.96, 0.95, 0.94, 0.93, 0.92, 0.91, 0.9]) * 0.95  # generate a random diagonal matrix
        ...
        # S_state_inf_intermediate = sc.linalg.solve_discrete_are(self.A.T, self.C.T, np.eye(nx) * sigma_w ** 2,
        #                                                         np.eye(ny) * sigma_v ** 2)
    ...
    def construct_C(A, ny,
                    normC):  # normC is a boolean that determines if the C matrix is sampled from a normal distribution or a uniform distribution
        nx = A.shape[0]
        _O = [np.eye(nx)]
        for _ in range(nx - 1):
            _O.append(_O[-1] @ A)
...
def generate_lti_system(C_dist, dataset_typ, nx, ny, sigma_w=1e-1, sigma_v=1e-1, n_noise=1):
    return FilterSim(nx, ny, sigma_w, sigma_v, tri=dataset_typ, C_dist=C_dist, n_noise=n_noise, new_eig=False)
