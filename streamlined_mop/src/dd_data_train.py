...
if __name__ == '__main__':
    ...
    for num_tasks in config.Ms:
        ...
        config.override("num_traces", {"train": config.total_training_traces // num_tasks, "val": config.num_traces["val"]})
        config_dict["num_traces"] = config.num_traces

        config.override("ckpt_path", output_dirs[num_tasks] + "/checkpoints/step=" + str(config.train_steps) + ".ckpt")
        ...
        train_gpt2(model, config, output_dirs[num_tasks])
        run.finish()
