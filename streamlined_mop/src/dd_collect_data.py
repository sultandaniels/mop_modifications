...
def collect_data(config, output_dirs):
    ...
    for M in Ms:
        ...
        for i in tqdm(range(config.num_val_tasks)):
            ...
            fsim, sample = generate_lti_sample(config.C_dist, config.dataset_typ, num_traces, config.n_positions, config.nx, config.ny, sigma_w=config.sigma_w, sigma_v=config.sigma_v, n_noise=config.n_noise, A=systems[i%config.num_val_tasks]["A"], C=systems[i%config.num_val_tasks]["C"]) # Don't oob when there are more validation systems than training tasks.
...
if __name__ == "__main__":
    config = Config()
    output_dirs = "streamlined_mop/ganguli_test"
