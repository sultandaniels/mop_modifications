import logging
from dyn_models import generate_lti_sample, generate_drone_sample, generate_lti_sample_steady  # , generate_pendulum_sample
    ...
    for name, num_tasks in zip(["train", "val"], [config.num_tasks, config.num_val_tasks]):
        ...
        for i in tqdm(range(num_tasks)):
            if config.dataset_typ == "drone":
                ...
                fsim, sample = generate_lti_sample_steady(config.dataset_typ, config.n_positions,
                                                   config.nx, config.ny,
